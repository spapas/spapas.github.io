...
- The client application creates a URL that contains the authorization request and redirects the user to the authorization endpoint of the authorization server
- The user logs in to the authorization server (or if he's already logged in he doesn't do anything)
- The user is redirected back to the client application with an authorization code
- The client application sends a request to the token endpoint of the authorization server with the authorization code and the client secret
- The authorization server responds with an access token
- The client application can now use the access token to access the user information using the userinfo endpoint

It is also explained in the section 1.3.  Overview of the [OpenID Connect specification](https://openid.net/specs/openid-connect-core-1_0.html#Overview).

# Authentication server

For the Authentication server for our tutorial we'll use [Keycloak](https://www.keycloak.org/), an open source Identity and 
Access Management server that implements OpenID Connect. I won't go into details on how to install and configure Keycloak,
...
If you are using a different OpenID Connect server, you'll need to have the client id and client secret for your server and the token, authorization and user info endpoints of your server.

# Automatic Discovery

Some OpenID Connect servers support automatic discover of various information from authorization server (including the needed endpoints). This is called `WebFinger` and is described in detail in the [discovery specification](https://openid.net/specs/openid-connect-discovery-1_0.html). 
...
- `response_type`: The response type, in our case it's `code`. There are various response types that can be used however `code` is the one that should be used to initiate the *Authorization Code Flow* which is used for server-side applications.
...
If we don't have anything running on localhost:8000 we'll get an error but we'll be able to see the redict URL on our browser's bar(!). So, we'll see something like If nothing is wrong, the user will be redirected to `http://localhost:8000/auth/callback?state=KpT23RpwimxzXzHa&session_state=d54e38e1-8dfa-4658-ab4e-817ef6d4029b&code=b4938b28-12b4-463b-b2bc-ffc91b29e79e.d54e38e1-8dfa-4658-ab4e-817ef6d4029b.f9302f16-4765-4f2a-b7bc-381208ec71d6`. So there are three parameters that are returned:

- `code`: The authorization code. This is the code that we'll use to get the access token.
...
a access token. For that it'll need to send a POST request to the token endpoint of the authorization server passing it the following parameters (see 3.1.3.1.  Token Request of the [OpenID Connect specification](https://openid.net/specs/openid-connect-core-1_0.html)):
...
Although there are libraries that can be used for decoding and verifying the token, we'll do it using only python to understand how it works.
...
- `iss`: The issuer of the token. This should be the same as the `OIDC_BASE_PROVIDER_URL` we defined earlier.
...
- `aud`: The audience of the token. This should contain the `OIDC_CLIENT_ID` we defined earlier (i.e sample-client).
